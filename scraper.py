...
OUTPUT_FILE = f"./data/scraped_jobs_{int(time.time())}.csv"
...
async def scrape(keys):
    ...
    with open(OUTPUT_FILE, mode="w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(["Job Title", "Company", "Posted Date", "URL"])  # Added URL column 🔥
            ...
            for page_num in range(1, last_page_number + 1):
                ...
                if job_blocks:
                    for job_block in job_blocks:
                        ...
                        job_titles = await job_block.query_selector_all("div.list-title a")  # Get the anchor tag 🔥
                        ...
                        for job, company in zip(job_titles, companies):
                            ...
                            job_url = await job.get_attribute("href")  # Get the job URL
                            ...
                            if not keys or any(k in job_title.lower() for k in keys):
                                print(f"Job Title: {job_title}, Company: {company_name}, Posted Date: {date_text}, URL: {job_url}")
                                writer.writerow([job_title, company_name, date_text, job_url])
